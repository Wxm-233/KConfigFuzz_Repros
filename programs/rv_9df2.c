...
void execute_call(int call)
{
  ...
  case 0:
    ...
    //    pid: pid (resource)
    //    file: ptr[in, buffer] {
    //      buffer: {73 74 61 63 6b 00} (length 0x6)
    //    }
    //  ]
    //  returns fd
    memcpy((void*)0x200000000000, "stack\000", 6);
    res = -1;
  ...
  case 1:
    ...
    //      buffer: (DirOut)
    //    }
    //    count: len = 0x5 (8 bytes)
    //  ]
    syscall(__NR_read, /*fd=*/r[0], /*buf=*/0x200000000040ul, /*count=*/5ul);
    break;
  case 2:
    //  setsockopt$MRT_INIT arguments: [
    //    fd: sock_igmp (resource)
    //    level: const = 0x0 (4 bytes)
    //    opt: const = 0xc8 (4 bytes)
    //    val: nil
    //    len: bytesize = 0x0 (8 bytes)
    //  ]
    syscall(__NR_setsockopt, /*fd=*/r[0], /*level=*/0, /*opt=*/0xc8,
            /*val=*/0ul, /*len=*/0ul);
    break;
  }
}
